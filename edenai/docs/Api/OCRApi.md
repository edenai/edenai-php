...
# **ocrCv**
> \edenai\Model\InlineResponse2005 ocrCv($files, $providers)
...
$files = "/path/to/file.txt"; // \SplFileObject | File to analyse (ex: pdf, doc, docs, rtf, png, jpg)
$providers = "providers_example"; // string | Providers to compare (ex: [ 'affinda', 'hrflow'])

try {
    $result = $apiInstance->ocrCv($files, $providers);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling OCRApi->ocrCv: ', $e->getMessage(), PHP_EOL;
}
?>
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **files** | **\SplFileObject**| File to analyse (ex: pdf, doc, docs, rtf, png, jpg) |
